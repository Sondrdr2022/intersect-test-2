...
from corridor_coordinator import CorridorCoordinator,IntelligentPhaseCoordinator
# ---- Config and Setup ----
...
# Replace the method body with this version
    ...
    def detect_congestion_patterns(self):
        """Detect congestion with lower thresholds for your scenario"""
        congestion_types = {
            'spillback': False,
            'gridlock': False,
            'arterial': False,
        ...
        for lane_id in self.lane_ids:
            ...
            # ADJUSTED: Lower thresholds
            if severity > 0.5:  # Reduced from 0.6
                congested_lane_count += 1
            
            # Spillback with lower threshold
            if queue_length > 0.6 * (lane_length / 7.5):  # Reduced from 0.7
                congestion_types['spillback'] = True
                
            # Gridlock detection
            if occupancy > 0.75:  # Reduced from 0.85
                downstream_lanes = self.get_downstream_lanes(lane_id)
                blocked_count = sum(1 for dl in downstream_lanes 
                                if traci.lane.getLastStepOccupancy(dl) > 0.6)
                if blocked_count > len(downstream_lanes) * 0.3:  # Reduced from 0.4
                    congestion_types['gridlock'] = True
        
        # CRITICAL: Force detection for your scenario
        avg_severity = total_severity / max(len(self.lane_ids), 1)
        
        # Much more aggressive critical detection
        if (avg_severity > 0.65 or  # Reduced from 0.75
            congested_lane_count > len(self.lane_ids) * 0.4 or  # Reduced from 0.6
            max_queue > 50):  # Direct queue threshold
            congestion_types['critical'] = True
            self.logger.info(f"[CRITICAL CONGESTION] Detected: avg_severity={avg_severity:.2f}, "
                            f"congested_lanes={congested_lane_count}, max_queue={max_queue}")
    ...
    def predict_congestion(self, lane_id, horizon=30):
        """Predict if congestion will occur in next 'horizon' seconds"""
        try:
            ...
            predicted_queue = current_queue + (arrival_rate - departure_rate) * horizon
            ...
            self.logger.info(f"Error predicting congestion: {e}")
            return False
    
    def calculate_departure_rate(self, lane_id):
        """Calculate vehicle departure rate when green"""
        if not self.is_lane_green(lane_id):
            return 0.0
        return 0.5  # vehicles per second
    ...
    def get_downstream_capacity(self, lane_id):
        """Get available space in downstream lanes"""
        try:
    ...
    def get_downstream_lanes(self, lane_id):
        try:
            return [link[0] for link in traci.lane.getLinks(lane_id) if link[0]]
        except Exception:
            return []
...
# --- PATCH 1: AdaptivePhaseController._safe_phase_index (force-reload option) ---
    ...
    def calculate_departure_rate(self, lane_id):
        ...
        # Typical value: 0.5 vehicles/second (1800 veh/hour)
        return 0.5
...
# Modify this method: detect_blocked_left_turn_with_conflict
    ...
    def control_step(self):
        ...
        # IMMEDIATE CHECK: Override for extreme queues
        max_queue = 0
        max_queue_lane = None
        for lane in self.lane_ids:
        ...
        # Force emergency if queue > 60 vehicles
        if max_queue > 60:
...
class UniversalSmartTrafficController:
    ...
    def __init__(self, sumocfg_path=None, mode="train", config=None, logger=None):
        ...
        self.corridor = CorridorCoordinator(
            self,
            spillback_queue_threshold=6.0,  # More sensitive
            downstream_pressure_threshold=4.0,  # Earlier detection
            wave_min_interval_s=8.0,  # Faster response
            congestion_prediction_horizon=30.0  # Look-ahead time
        )
